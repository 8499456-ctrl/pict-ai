...
async function processImage(request, env) {
  ...
  try {
    ...
    const models = {
      ...
      'product-poster': {
        model: 'black-forest-labs/flux-kontext-pro',
        input: { prompt: `Turn this exact product or object photo into a polished vertical product advertising poster suitable for Pinterest, ecommerce, and social media. Preserve the product identity, shape, material, color, proportions, packaging details, logo-like marks already on the product, and main camera angle. Create a clean commercial background, tasteful studio lighting, subtle shadows, and premium poster composition with space for headline text. Do not change the product into a different item. Do not invent fake brand names, readable marketing text, prices, watermarks, people, hands, or unrelated objects. Keep any existing product label as close to the source as possible, but do not add new text.${prompt ? ` Additional user direction: ${prompt}` : ''}`, input_image: dataUri, aspect_ratio: '4:5', output_format: 'jpg', safety_tolerance: 2, prompt_upsampling: false },
      },
      art: {
        model: 'black-forest-labs/flux-kontext-pro',
...
(function(){
  ...
  function applyCreativeCopy(tool,preserveValue){
    ...
    if(prompt){
      ...
      prompt.placeholder=tool==='remove-object'?tr('creative.placeholder.object'):tool==='product-poster'?'premium ecommerce poster, clean studio background, soft shadow, leave room for headline text':tr('creative.placeholder.scene');
